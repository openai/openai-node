...
| Monthly Node version review | OpenAI API key in the proposal job; repository and pull-request write authority only in the publisher               | Scheduled or manual main-only workflow; isolated proposal, validation, and publication jobs                                                                                             | Same immutable JSON artifact and workflow commit in fresh validation and publisher checkouts           | Codex action, read-only validation, and GitHub draft-PR publisher                           | Pinned actions, strict policy-file allowlist, engines.node-only package reconstruction, independent verification before dependency execution and publication, fixed validated base, and draft PR; human review and branch protection are external assumptions                                                               | `.github/workflows/node-version-review.yml:16-117`, `.github/workflows/node-version-review.yml:152-213`, `scripts/node-version-review.py`                                                                                                                                                                                                                                                                                                                                                             |
...
- **Workflow metadata or automation prompts to privileged writes:** the
  ...
  before writing statuses and PR comments. The monthly Node review isolates
  Codex with read-only repository access and an API key. Fresh validation and
  publisher jobs independently verify the same JSON proposal against the
  workflow commit, allowing only policy files and `package.json#engines.node`.
  Validation has read-only access; the publisher has repository/PR write access
  and applies verified data without installing dependencies or executing
  proposed code. Its local base remains at the validated commit. Required human
  review and branch protection are external assumptions
  (`.github/workflows/castiron-custom-code-comment.yml:4-24`,
  `.github/workflows/castiron-custom-code-comment.yml:116-200`,
  `.github/workflows/node-version-review.yml:16-117`,
  `.github/workflows/node-version-review.yml:152-213`,
  `scripts/node-version-review.py`).
- **Dependencies to build execution:** frozen lockfiles and workspace dependency
  ...
  (`.github/CONTRIBUTING.md:96-110`, `pnpm-workspace.yaml:7-37`).
