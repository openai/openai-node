...
A `buildRequest` override that copies options containing an already consumed one-shot header source
...
Foreign Headers-shaped implementations that return an empty replay after a nonempty first parse are
treated as exhausted one-shot sources. To intentionally clear all headers in such an implementation,
replace the header layer, or use an explicit record with `Authorization: null`; an empty replay alone
cannot establish that removal. Native `Headers`, arrays, and data records retain live refresh behavior.
