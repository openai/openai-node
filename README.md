...
Bun X.509 requests support HTTP CONNECT proxies, but reject HTTPS proxies (including inherited `HTTPS_PROXY`/`ALL_PROXY`) because Bun cannot isolate destination client-certificate TLS from proxy TLS. Native Undici `ProxyAgent` is not supported in X.509 mode because the SDK cannot prove that its hidden executable factories preserve one certificate across origins.
