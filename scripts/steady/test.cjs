...
async function health(port) {
  try {
    const response = await fetch(`http://127.0.0.1:${port}/_x-steady/health`, {
...
async function main() {
  ...
  rejects(/Steady source has local changes/u);
  ...
  rejects(/Deno executable checksum mismatch/u);
  ...
  rejects(/Deno archive checksum mismatch/u);
  ...
  child = spawn(
    'bash',
    [path.join(fixture, 'scripts/run-steady'), spec, '--host', '127.0.0.1', '-p', String(port)],
    {
      cwd: temporary,
  ...
  for (let attempt = 0; attempt < 150; attempt += 1) {
    // oxlint-disable-next-line no-await-in-loop -- Poll readiness in order.
    ...
    // oxlint-disable-next-line no-await-in-loop -- Bound the readiness polling rate.
    await delay(100);
  }
  ...
  console.log(
    'Pinned Steady: install, offline launch, integrity, relative paths, health, and cleanup passed.',
  );
}
