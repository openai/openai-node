...
async function main() {
  ...
  for (const code of ['EPERM', 'EACCES', 'EEXIST', 'ENOTEMPTY']) {
    ...
    assert.throws(
      () => publish('loser', linked),
      (actual) => actual === error,
    );
    assert.equal(fs.readFileSync(path.join(winner, 'content'), 'utf-8'), 'verified winner');
    assert.throws(
      () => publish('loser', path.join(root, 'absent')),
      (actual) => actual === error,
    );
    assert.throws(
      () => publish('loser', path.join(winner, 'content')),
      (actual) => actual === error,
    );
  }
  ...
  const attempts = Array.from({ length: 4 }, async (_, index) => {
  ...
  assert.match(fs.readFileSync(path.join(destination, 'content'), 'utf-8'), /^[0-3]$/u);
