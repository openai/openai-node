/** JSDoc import-reference parsing and lexical binding analysis for Oxlint. */
...
function getEntityRoot(entity) {
  const root = jsdocSymbols.getEntityRootNode(entity);
...
function isInJSDocExampleComment(text, example, tag) {
  let start = tag.pos;
  while (start > example.tagName.end && !/[\n\r\u2028\u2029]/u.test(text[start - 1])) start--;
...
function getCompilerJSDocCallables(documented) {
  if (ts.isVariableStatement(documented)) {
    return documented.declarationList.declarations
      .map((declaration) => declaration.initializer)
      .filter(ts.isFunctionLike);
  }

  let callable = documented;
  if (ts.isExportAssignment(documented)) {
    callable = documented.expression;
  } else if (
    ts.isExpressionStatement(documented) &&
    ts.isBinaryExpression(documented.expression) &&
    documented.expression.operatorToken.kind === ts.SyntaxKind.EqualsToken
  ) {
    callable = documented.expression.right;
  } else if (
    ts.isVariableDeclaration(documented) ||
    ts.isPropertyAssignment(documented) ||
    ts.isPropertyDeclaration(documented)
  ) {
    callable = documented.initializer;
  }
  return ts.isFunctionLike(callable) ? [callable] : [];
}
    ...
    if (!ts.isEmptyStatement(node)) {
      for (const comment of node.jsDoc ?? []) {
        if (
          ...
          comment.tags?.some(jsdocSymbols.isStandaloneJSDocDeclarationTag)
        ) {
          attached.set(comment.pos, { end: comment.end, node });
        }
      }
    }
...
function getJSDocImportUsage(context) {
  ...
  const { documentationNamespaces, localTypeBindings } = jsdocSymbols.getJSDocNamespaceInfo(compilerSource);

  for (const comment of sourceCode.getAllComments()) {
    if (comment.type !== 'Block' || !comment.value.startsWith('*')) continue;
    const compilerComment = attached.get(comment.range[0]);
    if (compilerComment?.end !== comment.range[1]) continue;
    ...
    if (!doc) continue;
    const callables = getCompilerJSDocCallables(compilerComment.node);
    ...
    for (const originalTag of tags) {
      ...
      if (inExample && !startsLine) {
        const parameter =
          ts.isJSDocParameterTag(originalTag) &&
          callables.some((callable) =>
            callable.parameters.some((candidate) =>
              ts
                .getJSDocParameterTags(candidate)
                .some((tag) => tag.pos === comment.range[0] + originalTag.pos),
            ),
          );
        if (!parameter || isInJSDocExampleComment(parsed.text, exampleTag, originalTag)) continue;
        inExample = false;
      }
      if (inExample && startsLine) inExample = false;
      if (name === 'example') {
        inExample = true;
        exampleTag = originalTag;
        continue;
      }
...
module.exports = { getEntityRoot, getJSDocImportUsage };
