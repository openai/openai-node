...
if (require.main === module) {
  test('Ultracite checks handwritten imports and skips Stainless-generated files', () => {
    ...
    const generated = ['src/client.ts', 'src/internal/headers.ts', 'tests/index.test.ts'];
    const handwritten = ['src/internal/uploads.ts', 'tests/generated-resource-helpers.test.ts'];

    for (const configuration of [lintConfiguration, formatConfiguration]) {
      for (const file of generated) assert.ok(configuration.ignorePatterns.includes(file), file);
      for (const file of handwritten) assert.ok(!configuration.ignorePatterns.includes(file), file);
    }

    const selected = [generated[0], handwritten[0]];
    const linted = run(oxlint, ['--format', 'json', '--no-error-on-unmatched-pattern', ...selected]);
    assert.equal(JSON.parse(linted.stdout).number_of_files, 1);

    const oxfmt = path.join(repoRoot, 'node_modules', '.bin', 'oxfmt');
    const formatted = run(oxfmt, ['--check', '--no-error-on-unmatched-pattern', ...selected]);
    assert.match(formatted.stdout, /on 1 files?\b/);

    const ultracite = path.join(repoRoot, 'node_modules', '.bin', 'ultracite');
    const fixture = writeFixture(
      'ultracite-imports.js',
    ...
    const result = spawnSync(ultracite, ['check', path.relative(repoRoot, fixture)], options);

    assert.notEqual(result.status, 0);
    assert.match(result.stdout, /sdk\(no-unused-imports\)/);
    assert.doesNotMatch(result.stdout, /no-unused-vars/);

    const fixed = spawnSync(ultracite, ['fix', path.relative(repoRoot, fixture)], options);
    assert.equal(fixed.status, 0, `${fixed.stdout}\n${fixed.stderr}`);
    assert.doesNotMatch(fs.readFileSync(fixture, 'utf8'), /import \{ Foo \}/);
    assert.match(fs.readFileSync(fixture, 'utf8'), /intentionallyUnused/);
  });

  require('./oxlint-review-regression.test.cjs');
  require('./oxlint-jsdoc-example-regression.test.cjs');
  require('./oxlint-import-regression.test.cjs');
