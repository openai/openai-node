...
X.509 workload identity requires a static Undici `Agent` TLS configuration with the default origin factory. Load PEM certificate, key, and CA files as UTF-8 strings. Dynamic connectors, custom origin factories, `ProxyAgent`, mutable TLS containers such as `Buffer`s or arrays, and opaque executable dispatchers are deliberately unsupported because the SDK cannot prove that token exchange and API dispatch will present the same certificate. Bun X.509 mode supports HTTP CONNECT proxies but rejects HTTPS proxies, including inherited proxy environment variables.
