...
test.each([null, ''] as const)(
  'retains a one-shot authorization override after prepareOptions inspection: %j',
  async (authorization) => {
    const options: FinalRequestOptions = {
      method: 'post',
...
test.each(['mutate', 'replace', 'delete'] as const)(
  'preserves prepareOptions header changes after the first snapshot: %s',
  async (change) => {
...
test('snapshots a replacement one-shot header source before retry preparation', async () => {
  const options: FinalRequestOptions = {
    method: 'post',
    path: 'https://synthetic.example.test/resource',
    body: { synthetic: true },
