...
test.each(restrictedHeaders)('rejects legacy $name before dispatch', ({ headers }) => {
  const { client, fetch } = createClient();

  expect(() => client.files.list({ headers })).toThrow(/explicit request options argument/u);
  expect(fetch).not.toHaveBeenCalled();
});

test('protects bearer, beta, and admin GET entrypoints', () => {
  const { client, fetch } = createClient();
  const options = { headers: { 'OpenAI-Project': 'proj_other' } };

  expect(() => client.responses.retrieve('resp_synthetic', options)).toThrow(TypeError);
  expect(() => client.beta.assistants.list(options)).toThrow(TypeError);
  expect(() => client.admin.organization.projects.list(options)).toThrow(TypeError);
  expect(fetch).not.toHaveBeenCalled();
});
...
test('dispatches the validated header snapshot after caller mutation', async () => {
  const { client, fetch } = createClient();
  const headers = new Headers({ 'x-custom': 'original' });
  const pending = client.files.list({ headers });
  headers.set('x-custom', 'changed');
  headers.set('OpenAI-Project', 'proj_other');
