...
describe('Bedrock provider optional dependencies', () => {
  test('keeps the root and bearer entrypoints independent from AWS packages', async () => {
    ...
    const openai = await import('openai');
    const bearerProvider = await import('openai/providers/bedrock');
  ...
  test.each(['mantle', 'runtime'] as const)(
    'resolves temporary environment credentials through the real AWS default chain for %s',
    async (endpoint) => {
      ...
      const client = new OpenAI({
        provider: bedrock({ endpoint, region: 'us-west-2', apiKey: null }),
        maxRetries: 0,
      ...
      expect(headers.get('authorization')).toContain(
        `/us-west-2/${endpoint === 'runtime' ? 'bedrock' : 'bedrock-mantle'}/aws4_request`,
      );
  ...
  test.each(['mantle', 'runtime'] as const)(
    'forwards a named profile to the AWS default provider and signs the %s request',
    async (endpoint) => {
      const credentialsProvider = vi.fn(async () => ({
      ...
      let requestedInit: RequestInit | undefined;
      const client = new OpenAI({
        provider: bedrock({ endpoint, region: 'us-west-2', profile: 'engineering' }),
        maxRetries: 0,
        fetch: async (_url: RequestInfo, init?: RequestInit) => {
          requestedInit = init;
          return jsonResponse();
  ...
  test.each(['mantle', 'runtime'] as const)(
    'rejects an invalid identity resolved by the default AWS provider for %s',
    async (endpoint) => {
      ...
      const fetch = vi.fn(async () => jsonResponse());
      const client = new OpenAI({
        provider: bedrock({ endpoint, region: 'us-east-1', profile: 'invalid-profile' }),
        maxRetries: 0,
        fetch,
      });
  ...
  test('surfaces the runtime module error when an AWS dependency is missing', async () => {
    ...
    await expect(loadBedrockModules()).rejects.toMatchObject({ cause: missingDependency });
  });

  test.each(['mantle', 'runtime'] as const)(
    'preserves the default credential chain failure and its cause for %s',
    async (endpoint) => {
      ...
      const fetch = vi.fn(async () => jsonResponse());
      const client = new OpenAI({
        provider: bedrock({ endpoint, region: 'us-east-1', apiKey: null }),
        maxRetries: 0,
        fetch,
      });
