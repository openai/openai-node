...
describe('toResponseInputItems', () => {
  ...
  test('normalizes current output-only fields and omits non-replayable items', () => {
    ...
    expect(() =>
      // oxlint-disable-next-line anti-slop/no-chained-type-assertions -- An unsupported future discriminator must reach the runtime conversion boundary and be rejected.
