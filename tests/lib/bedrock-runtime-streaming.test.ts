...
const RUNTIME_RESPONSES_URL = 'https://bedrock-runtime.us-east-1.amazonaws.com/openai/v1/responses';
const RUNTIME_PROVIDERS = [
  {
    authentication: 'bearer',
    create: () => bearerBedrock({ endpoint: 'runtime', region: 'us-east-1', apiKey: 'bedrock-token' }),
...
describe('bedrock Runtime streaming', () => {
  test.each(RUNTIME_PROVIDERS)(
    'streams Chat Completions in wire order with $authentication authentication',
    async ({ create, authorization }) => {
      ...
      expect(request?.headers.get('authorization')).toContain(authorization);
  ...
  test.each(RUNTIME_PROVIDERS)(
    'uses the terminal Responses event with $authentication authentication',
    async ({ create, authorization }) => {
      ...
      expect(request?.url).toBe(RUNTIME_RESPONSES_URL);
      expect(JSON.parse(request?.body ?? '{}')).toMatchObject({ model: RUNTIME_MODEL, stream: true });
      expect(request?.headers.get('authorization')).toContain(authorization);
