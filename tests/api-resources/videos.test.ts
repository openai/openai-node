...
describe('resource videos', () => {
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.videos.list({
    after: 'after',
    limit: 0,
    order: 'asc',
  ...
  test('createCharacter: only required params', async () => {
    const responsePromise = client.videos.createCharacter({ name: 'x', video: await toFile(Buffer.from('Example data'), 'README.md') });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('createCharacter: required and optional params', async () => {
    const response = await client.videos.createCharacter({ name: 'x', video: await toFile(Buffer.from('Example data'), 'README.md') });
  });

  test('downloadContent: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.videos.downloadContent('video_123', { variant: 'video' }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(OpenAI.NotFoundError);
  });

  test('edit: only required params', async () => {
    const responsePromise = client.videos.edit({ prompt: 'x', video: await toFile(Buffer.from('Example data'), 'README.md') });
    const rawResponse = await responsePromise.asResponse();
    expect(rawResponse).toBeInstanceOf(Response);
    const response = await responsePromise;
    expect(response).not.toBeInstanceOf(Response);
  ...
  test('edit: required and optional params', async () => {
    const response = await client.videos.edit({ prompt: 'x', video: await toFile(Buffer.from('Example data'), 'README.md') });
  });
