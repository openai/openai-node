...
describe('resource items', () => {
  test('create: only required params', async () => {
    const responsePromise = client.conversations.items.create('conv_123', { items: [{
    content: 'string',
    role: 'user',
    type: 'message',
  }] });
  ...
  test('create: required and optional params', async () => {
    const response = await client.conversations.items.create('conv_123', { items: [{
    content: 'string',
    role: 'user',
    phase: 'commentary',
    type: 'message',
  ...
  test('retrieve: required and optional params', async () => {
    const response = await client.conversations.items.retrieve('msg_abc', { conversation_id: 'conv_123', include: ['file_search_call.results'] });
  });
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.conversations.items.list('conv_123', {
    after: 'after',
    include: ['file_search_call.results'],
    limit: 0,
    order: 'asc',
