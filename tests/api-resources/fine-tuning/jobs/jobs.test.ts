...
describe('resource jobs', () => {
  test('create: only required params', async () => {
    const responsePromise = client.fineTuning.jobs.create({ model: 'gpt-4o-mini', training_file: 'file-abc123' });
  ...
  test('list: request options and params are passed correctly', async () => {
    ...
    await expect(client.fineTuning.jobs.list({
    after: 'after',
    limit: 0,
    metadata: { foo: 'string' },
  ...
  test('listEvents: request options and params are passed correctly', async () => {
    // ensure the request options are being passed correctly by passing an invalid HTTP method in order to cause an error
    await expect(client.fineTuning.jobs.listEvents('ft-AF1WoRqd3aJAHsqc9NY7iL8F', { after: 'after', limit: 0 }, { path: '/_stainless_unknown_path' }))
      .rejects
      .toThrow(OpenAI.NotFoundError);
  });
