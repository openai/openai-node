...
export class ServiceAccounts extends APIResource {
   ...
   * ```ts
   * const response =
   *   await client.admin.organization.projects.serviceAccounts.createAPIKey(
   *     'service_account_id',
   *     { project_id: 'project_id' },
   *   );
   * ```
  ...
  createAPIKey(
    ...
    options?: RequestOptions,
  ): APIPromise<ServiceAccountCreateAPIKeyResponse> {
    const { project_id, ...body } = params;
    return this._client.post(
...
export interface ServiceAccountCreateAPIKeyResponse {
  /**
...
export interface ServiceAccountCreateAPIKeyParams {
  /**
   * Path param: The ID of the project.
   */
  project_id: string;
