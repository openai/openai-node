...
import { LiveEmitter, LiveStreamMessage, WebSocketError, buildURL } from './internal-base';
...
export abstract class LiveWSBase<TSocket extends WebSocketLike> extends LiveEmitter {
  ...
  private _sendQueue: SendQueue<LiveAPI.ClientEvent>;
  ...
  constructor(client: OpenAI, options?: LiveWSBaseOptions | undefined) {
    ...
    this._sendQueue = new SendQueue<LiveAPI.ClientEvent>(options?.maxQueueSize);
  }
  ...
  send(event: LiveAPI.ClientEvent) {
    if (!this.socket) {
  ...
  [Symbol.asyncIterator](): AsyncIterableIterator<LiveStreamMessage> {
    ...
    const queue: (LiveStreamMessage | undefined)[] = [];
    ...
    const onEvent = (event: LiveAPI.ServerEvent) => {
    ...
    const onClose = (code: number, reason: string, unsent: UnsentMessage<LiveAPI.ClientEvent>[]) => {
    ...
    const resolve = (res: (value: IteratorResult<LiveStreamMessage>) => void) => {
  ...
  private _connect(): TSocket {
    ...
    socket.on('message', (data: string | ArrayBuffer | ArrayBufferView, isBinary: boolean) => {
      ...
      let event: LiveAPI.ServerEvent;
      try {
        event = JSON.parse(text) as LiveAPI.ServerEvent;
      } catch {
        this._emit('raw', data);
        return;
      }
