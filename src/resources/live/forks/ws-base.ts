...
import { ForksEmitter, ForksStreamMessage, WebSocketError, buildURL } from './internal-base';
...
export interface ForksWSReconnectOptions {
   ...
   */
  onReconnecting(
    event: ReconnectingEvent<ForksWSParameters>,
  ): ReconnectingOverrides<ForksWSParameters> | void;
...
export abstract class ForksWSBase<TSocket extends WebSocketLike> extends ForksEmitter {
  url!: URL;
  socket!: TSocket;

  protected _client: OpenAI;
  protected _parameters: ForksWSParameters | null | undefined;
  private _reconnectOptions: ForksWSReconnectOptions | null;
  private _sendQueue: SendQueue<ForksAPI.ForkClientEvent>;
  ...
  constructor(client: OpenAI, parameters: ForksWSParameters, options?: ForksWSBaseOptions | undefined) {
    ...
    this._sendQueue = new SendQueue<ForksAPI.ForkClientEvent>(options?.maxQueueSize);
  }
  ...
  send(event: ForksAPI.ForkClientEvent) {
    if (!this.socket) {
  ...
  [Symbol.asyncIterator](): AsyncIterableIterator<ForksStreamMessage> {
    ...
    const queue: (ForksStreamMessage | undefined)[] = [];
    ...
    const push = (msg: ForksStreamMessage) => {
      if (done) return;
    ...
    const onEvent = (event: ForksAPI.ForkServerEvent) => {
    ...
    const onReconnecting = (evt: ReconnectingEvent<ForksWSParameters>) => {
    ...
    const onClose = (code: number, reason: string, unsent: UnsentMessage<ForksAPI.ForkClientEvent>[]) => {
    ...
    const resolve = (res: (value: IteratorResult<ForksStreamMessage>) => void) => {
    ...
    const next = (): Promise<IteratorResult<ForksStreamMessage>> =>
      new Promise((res) => {
  ...
  private _connect(): TSocket {
    ...
    socket.on('message', (data: string | ArrayBuffer | ArrayBufferView, isBinary: boolean) => {
      ...
      let event: ForksAPI.ForkServerEvent;
      try {
        event = JSON.parse(text) as ForksAPI.ForkServerEvent;
      } catch {
        this._emit('raw', data);
        return;
      }
  ...
  private async _reconnect(closeCode: number): Promise<void> {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      ...
      let reconnectingEvent: ReconnectingEvent<ForksWSParameters> = {
        attempt,
      ...
      let overrides: ReconnectingOverrides<ForksWSParameters> | void;
      try {
