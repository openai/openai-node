...
import type { MultipartDataSnapshot } from './multipart-snapshot';
import { snapshotBlobData, snapshotResponseData, snapshotStreamingFileData } from './multipart-snapshot';
...
export function getName(value: any, options?: { stripFilename?: boolean | undefined }): string | undefined {
  return getNameInternal(value, options);
}

function getNameInternal(
  value: unknown,
  options?: { stripFilename?: boolean | undefined },
  knownNamedBlob = false,
): string | undefined {
  ...
  const explicitName =
    ((knownNamedBlob || 'name' in candidate) && candidate.name ? String(candidate.name) : undefined) ||
    ('filename' in candidate && candidate.filename ? String(candidate.filename) : undefined);
  if (explicitName) {
    return options?.stripFilename === false ? normalizeFilenamePath(explicitName) : basename(explicitName);
  }

  const url = 'url' in candidate && candidate.url ? String(candidate.url) : undefined;
  if (url) {
    try {
  ...
  const path = 'path' in candidate && candidate.path ? String(candidate.path) : undefined;
...
type UploadableKind = 'named-blob' | 'response' | 'streaming-upload' | 'streaming-file' | undefined;
...
const hasStreamingUploadableValue = (value: unknown, uploadableKinds: UploadableKinds): boolean => {
  ...
  if (uploadableKind === 'named-blob' || uploadableKind === 'response') {
    return false;
  }
...
const addFormValue = async (
  ...
  const cachedUploadableKind = typeof value === 'object' ? uploadableKinds?.get(value) : undefined;

  // Nested form keys use the current bracketed encoding.
  if (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean') {
    form.append(key, String(value));
  } else if (cachedUploadableKind === 'response' || value instanceof Response) {
    const response = value as Response;
    form.append(
      key,
      makeFile([await response.blob()], getName(response, { stripFilename: options.stripFilenames })),
    );
  } else if (cachedUploadableKind === 'named-blob') {
    const blob = value as NamedBlob;
    form.append(key, blob, getNameInternal(blob, { stripFilename: options.stripFilenames }, true));
  } else if (isAsyncIterable(value)) {
    form.append(
      key,
      makeFile(
